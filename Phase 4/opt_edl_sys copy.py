...
experiment, end_event = experiment3()
...
print(max(rover['telemetry']["Time"] ))
bj_f = obj_fun_time(x0,edl_system,planet,mission_events,tmax,
                               experiment,end_event)

print(bj_f)
bounds = [[14, 0.2, 250, 0.05, 100], [19, 0.7, 800, 0.12, 290]]
#print(bounds[0])
timechange = []
v = []
n = ["Parachute Diameter", "Wheel Diameter", "Chassis Mass","Gear Diameter","Fuel Mass"]
plot1 = plt.figure(0)
fig, axs = plt.subplots(5,1,sharex=True )
plot1.tight_layout()
x0 = bounds[0]
for  i in range(len(bounds[0])):
    var = np.linspace(bounds[0][i],bounds[1][i], 10)
    print(var)
    t = []
    for x in var:
        x0[i] = x
        bj_f = obj_fun_time(x0,edl_system,planet,mission_events,tmax,experiment,end_event)
        t.append(bj_f)
    x0 = bounds[1]
    #plt.plot(t, var, "b-")
    #plt.title()
